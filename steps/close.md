...
| **L1·致命** | 前置证据不满足（§2 在线证据核验失败，如预期提交不可从目标 ref 到达 / 活动 checkout 仍含未保存唯一修改） | 报错退出，报告缺失证据，**用户说"已提交"不是跳过 git 证据校验的理由** |
...
## §2 前置证据核验（close 前必须全部确认）

1. 用户明确声明修改已提交或在线分支已更新（声明是**启动检查的授权**，不是跳过证据校验的理由）
2. 目标在线 ref 和 commit 可识别（如 `origin/master` → commit）
3. **本工单预期提交确实可从目标 ref 到达**：`git merge-base --is-ancestor <活动 checkout 分支 HEAD> <目标 ref>`（退出码 0 = 可达；只读命令）。不可达 → 报告差异并让用户确认实际落地 commit（可选指定）；**不得跳过**
...
5. **记录后续维护基线**：`submitted_baseline` = 目标 ref 实际解析 commit（**用 commit 不用分支名**）
...
10. **更新 metadata + index**：`checkout_history` 中已清理项的 `removed_at=<ts>`、`submitted_baseline`、`active_checkout=null` 同步写 metadata + 全局 index（写前重读合并契约见 [references/dir_and_metadata.md「全局索引写入」](../references/dir_and_metadata.md)）
11. **输出**：保留产物路径（`archive_path`）、在线基线（`submitted_baseline`）、未清理资源清单（未能安全删除的分支/worktree）
...
submitted baseline commit != null
...
重复执行 close：
- 已 close（`submitted_baseline` 非 null 且 `active_checkout` null）→ 报告「本工单已关闭，基线 {commit}；如需恢复请 /icode worktree --reopen」，不重复清理
...
- **禁止伪造关闭**：`submitted_baseline` 必须是实际核验的 commit，不是用户口头声称的分支名
- **禁止真实项目术语**：示例/输出用通用占位符

## MCP 推荐

close 是结构化状态机执行，不需要 spawn 子代理；仅用 sequential-thinking（强制思考前置：证据核验 → 清理顺序 → 残留处理 → 风险，≥4 步）。其余 MCP 不推荐。
