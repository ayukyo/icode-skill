...
> **本质**：本步骤是「内容组织 + 模板替换」两步。内容**必须**来自 icode 已有产物/知识库（先收集再组织），模板**只替换文字不破坏排版**（内置 8 套模板，见 [tools/ppt/](../tools/ppt/README.md)）。
...
1. 读 [tools/ppt/templates/INDEX.md](../tools/ppt/templates/INDEX.md)（8 套模板的风格/主色/适用场景/页数，每套配 `preview.png`）。
2. 按「场景 + 风格」从 8 套里筛 **2-3 个候选**：候选必须实际存在且与本次场景匹配，优先选择不同风格/主色；匹配不足时如实少列，不为凑数量推荐不合适的模板。参考：汇报/复盘类 `minimal-business-summary`、`premium-corp`、`competition-speech`；数据密集 `data-viz-deck`、`mckinsey-style`；架构讲解 `architecture-deck`、`mckinsey-style`、`premium-corp`；学术 `thesis-novice`、`thesis-formula`。把每套的一句话特点 + 推荐理由一并列出。
3. **让用户在候选中挑一套**（用户直接指定模板名 → 跳过候选、直接用）。选定后才进入 §3；不要把选择结果锁死在某一套上。
4. **指定模板缺失**：用户或既有 edits 指定的模板不在当前安装中时，明确列出可用替代项，等待用户选款或提供原模板；保留原 edits/成品，不静默换模板，不自动下载已移除资源。
...
内置 8 套模板来自第三方设计师作品，**仅供个人学习与非商业研究，禁止商业用途**，详见 [tools/ppt/NOTICE](../tools/ppt/NOTICE)及其中的上游许可来源。个人学习与非商业研究不以购买商业授权为前置；无可见水印不表示无使用限制。生成脚本（`tools/ppt/scripts/`）为 MIT；用户自备模板按其自身许可，不将内置模板限制笼统套用到全部 ICODE 或所有成品。
