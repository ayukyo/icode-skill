...
- **无描述** (`/icode limit`) → 全局扫描：合并显示当前约束（main + local 覆盖后），不做任何写入
...
- 主存 main 存在 → 读其所有条目
- 覆盖 local 存在 → local 条目**完全替换**同名条目（按"红线编号"或"约束标题"匹配），未匹配的 main 条目保留
- local 不存在 → 仅显示 main
- main 不存在 → 提示"⚠️ 本工程尚无 limit 约束，建议运行 `/icode limit <约束描述>` 生成"

> **简化实现**：为避免"按编号/标题匹配"复杂度，**本实现采用"整文件覆盖"语义**——local 存在时把 local 内容当作完整 limit 视图；local 不存在时显示 main。**理由**：limit 条目通常不多（典型 5~20 条），复杂合并规则维护成本高于收益。如未来需要精细合并可迭代。
