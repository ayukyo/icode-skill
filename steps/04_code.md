...
> **共享技能路由**：编码准入三链预扫前读取 [references/skill_routing.md](../references/skill_routing.md)；仅加载与本次调用链、状态或交付风险匹配的技能。
...
**日志准确性核对**：对新增/修改的result、状态、计数日志逐分支记录真实条件来源；未启动/未执行不能写joined/synced=1。导出Result枚举逐分支核对线程结束与操作成功语义；原始syscall errno和策略reason分开；同类checked IO wrapper对照成功/失败errno约定。必要故障注入只覆盖真实源码路径。

**偏差单源**：实际偏离先通过metadata-update写code_deviations（稳定id、plan_said/actual_done/reason及source_step/plan_ref/code_ref），03/04/05/06引用同id；无偏差才用[]。终审交叉核对各报告与机器数组，不使用关键词正则猜偏差。历史缺记允许注明事后补记时间/来源，不声称当时已有回执。
