...
4. **应用契约**：plan §3 架构设计 / §4 ADR / §6 异常处理须呼应相关 limit 条目（"本工程 limit 红线 N：X，本方案选择 Y 因为..."）；可在 plan metadata 写入 `limit_refs` 数组（可选，含引用的红线编号 + 引用章节）
...
5. 撰写计划：
   a. **先了解现有工程**：阅读项目中现有的代码，了解目录结构、现有架构模式、可复用模块 **serena 优先（v2.2 执行步骤内嵌）**：若工程有可索引源码（.py/.ts/.js/.c/.cpp/.rs/.go/.java 等）且 serena 可用（`~/.claude.json` 注册 + deferred 列表有 `mcp__serena__find_symbol`），先 ToolSearch 取 schema -> `find_symbol` 找 entry/导出符号 -> `find_referencing_symbols` 摸清关键调用链，结果作为 §1.5 工程结构快照输入；目标代码在子仓库/嵌套 git 仓库时（`git -C <dir> rev-parse --show-toplevel` ≠ 当前激活项目根），先 `serena-doctor init/fix` 子仓库根 + `activate_project(<子仓库根>)` 激活目标仓库再查（v2.12，详见 anti_laziness 第 21 条 v2.12 段）；激活失败才降级。serena 不可用/无 LSP -> 降级 Read+Grep，降级说明只进思考块，不写入产物文件。**未经实际调用 serena 就标降级 = 反偷懒第 21 条违规。**步骤末尾按反偷懒第 21 条 v2.6 自检门输出 `serena 调用: <工具 x N>` 或 `serena 降级: <原因>`，无记录 = 违规****。
...
4. **架构决策记录（ADR）** — 每个关键决策记录：上下文、候选选项、决策、理由。格式如下：
   ...
   - **现有功能覆盖度检查**（v2.11，涉及新增功能/新增接口/新增模块的 ADR 必做）：按 [references/necessity_check.md](../references/necessity_check.md) 全工程检索确认无等价实现，结论写 `未覆盖` / `部分（边界）` / `已覆盖（file:line）`。纯内部重构（无新增功能语义）标「本 ADR 无新增功能语义，覆盖度检查不适用」。
   ...
   需记录 ADR 的典型场景：方案选型、降级策略、接口取舍、兼容性权衡、**跨模块/跨端点调用方式选择**（直调 vs 路由/事件分发）、**跨层翻译入口点选择（v2.4 新增，在哪个边界做归一化：数据入口适配器 vs N 处散补丁）**。决策变更时只需更新此章节 + 受影响的引用点。
...
### 状态机链路推演（涉及状态机/多态流程修改时必填，v2.14 新增）
...
> **分档结构（v2.7 反偷懒第 26 条）**：本节按 A 档（根因修复，主方案）/ B 档（兜底防御，单独小节，标注"A 修复后触发概率"）/ C 档（后续工单，范围外）分列。§3 架构设计只含 A 档。4 维度设计态主要固化 A 档。
...
- **维度 2 逻辑+扩大修改设计**：本修复涉及的边界/状态/异常/时序/数值 5 类逻辑点预判 + 预期修改范围（最小侵入边界，列出 file:line 候选）+ 优雅度6条预判（复用/风格/调用链/最小侵入/接口克制/调用路径）。**（v2.4 增补）状态类的「跨模块枚举对齐」子项**：本修改涉及的状态值若来自外部模块（SDK/其他进程/共享库），必须对照上下游枚举定义并记录「同名数值是否同义 / 哪些是过渡态 vs 终局态」——避免「语义碰撞」型根因。修复方案优先选「数据入口适配器（一次归一化，下游不感知）」，避免「N 处散补丁」。**log 工单本子项的设计起点 = `log_analysis.md §2.2 跨模块枚举对照表`（已在阶段1基线检查生成）**；init 工单无此表，按上述指引独立设计
...
- 校验项以 [ ] checkbox 形式列出
   ...
   **与 §4.5 4 维度设计态的关系**：
   ...
   c. **断言验证**（必须执行，不可跳过）：
      ...
      - **覆盖类**（v2.11 新增）：对计划每个"新增功能点/新增 public 接口"，断言"工程现有代码无等价实现"——按 [references/necessity_check.md](../references/necessity_check.md) 全工程检索 + Read 命中处行为链（证据 file:line）。验证失败（发现等价实现）→ 标 `[未验证-发现现有等价实现]`，**计划修正（需求点改复用/删除），不是修断言**
      - **首次激活路径一致性类**（v2.14 新增）：当修改涉及一条**从未被实机执行过**的历史代码路径时，核对**跨层接口双侧校验一致性**。定义与完整核对清单见 [references/first_activation_path.md](../references/first_activation_path.md)。对 plan 涉及的每个跨层接口 / 请求 action：grep 历史日志确认是否有**成功调用记录**；若无（首次激活路径）→ 断言"请求方入参校验 与 接收方校验 对同一字段的接受值要求一致"，Read 两侧逐字段对比 → 不一致标 `[未验证-双侧校验矛盾]`，**计划必须同时修两侧**。**软信号**：历史日志可能不全，命中只触发核对，不阻断；严禁当硬性失败
   ...
   c.5 **方案对抗验证**（plan 阶段反推核心机制，治「自验自证」盲区）：
      ...
      - **三视角质疑者**（任一独立 spawn，禁用 Explore，用 general-purpose + schema 强制结构化）：
        ...
        - **兼容性质疑者**：方案是否破坏既有功能？是否引入回归？上下游依赖是否被忽略？API 变更是否兼容？**方案必要性（v2.11）**：方案是否在解决一个已被现有代码解决的问题？输入文件中是否有与方案功能重叠的现有实现？——若发现重叠，必须质疑方案必要性并回指 file:line
        - **调用链完整性质疑者**：方案涉及的所有跨模块/跨端点调用，调用链是否完整？返回路径是否闭环？异常分支是否都有出口？
      - **输入契约**（v2.11 增补）：质疑者输入 = 方案涉及的文件 + **`rg -in '<需求关键词>'` 命中的非计划文件**（全工程检索，含计划未涉及模块）。质疑者看不到重叠文件就永远不会质疑必要性——grep 命中文件必须喂给质疑者
      - **收敛与降级**：
