...
**输出方式（必须遵守）**：
1. 使用 Write 工具将完整计划写入 `{ICODE_OUT_DIR}/01_plan.md`
2. 如果写入失败，**必须多次重试**直到成功
3. 写入成功后，回复"已完成：{ICODE_OUT_DIR}/01_plan.md"，不要输出计划内容本身
...
4. **确认子 Agent 已将计划写入 `{ICODE_OUT_DIR}/01_plan.md`**（检查文件存在且非空，缺失则重新启动子 Agent）
