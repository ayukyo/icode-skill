...
**修复方案三档分级（v2.7，反偷懒第 26 条）**：本节修复方案分 A 档（根因修复，主方案，必做）/ B 档（兜底防御，单独小节，标注"A 修复后触发概率"，用户显式确认才实施）/ C 档（后续工单，进范围外）。§3 架构设计只含 A 档改动，B 档不混入。ADR：A 档多候选走 A1/A2 对比，B 档不进 ADR。A 档跨工程（修复点不在当前工程）时 plan 明示"本工程无可实施 A 档，转交 <X> 工单"，不把 B 档提为 A 档。最小修复=只做 A 档。详见 anti_laziness 第 26 条
   ...
   **与 §4.5 4 维度设计态的关系**：
   ...
   c.5 **方案对抗验证**（plan 阶段反推核心机制，治「自验自证」盲区）：
      ...
      - **三视角质疑者**（任一独立 spawn，禁用 Explore，用 general-purpose + schema 强制结构化）：
        ...
        - **兼容性质疑者**：方案是否破坏既有功能？是否引入回归？上下游依赖是否被忽略？API 变更是否兼容？
