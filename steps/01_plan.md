...
3. **存在性分支**：
   ...
   - **存在** → Read 主存 + 覆盖（**local 完全覆盖 main**，同 limit 步骤合并逻辑），结果进 plan 步骤的思考分级（L2）块作硬基线
...
3. **思考分级（L2：sequential-thinking）**（按 [references/mcp_per_step.md](../references/mcp_per_step.md)「通用前置·分级思考」段执行）：本步骤为 L2，调用 sequential-thinking（3~5 步）。思考职责 = 需求分解 → 方案分析 → 风险评估。**若步骤2有历史参考，在此处「历史参考」小节记录命中工单 id 与 ADR/风险要点，作为思考输入**
4. 自动迁移（如上「## 前置：schema 迁移」段）—— 迁移到 v1.1
5. 撰写计划：
   a. **先了解现有工程**：阅读项目中现有的代码，了解目录结构、现有架构模式、可复用模块（**grep 优先**）：`grep -rn '<entry>/<导出符号>'` 定位入口与导出符号、`grep -rn '<符号>('` 摸清关键调用链（跨仓库/子仓库见 反偷懒第 21 条「跨仓库/子仓库检索」段），结果作为 §1.5 工程结构快照输入。检索结果只进思考块，不写入产物文件。
...
4. **架构决策记录（ADR）** — 每个关键决策记录：上下文、候选选项、决策、理由。格式如下：
   ...
   - **结构性机制先例核查（O-4，并入本字段）**：当设计含**结构性机制**（事件化消费、缓存+退避、锁序、快照读、状态机迁移、幂等重试等）时，`rg` 全仓检索既有同类实现（关键词：事件消费原语 / 退避 / 锁序相邻 / 状态机迁移等）——**命中先例 → 新实现必须与先例同构**（命名/结构/消费方式对齐；实现时"巧合对齐"不算主动对齐，必须设计态就核对）；**未命中 → 计划须自证**该机制与既有架构一致，或说明为何引入新模式。**锁序先例（防 ABBA）**：新加锁必须 grep 既有锁获取顺序，沿既有单向锁序延伸，绝不反向加锁（与 O-5 线程归属矩阵联动）。
...
- **维度 3 竞态死锁设计**：本修复涉及的 10 条强制清单（按工程相关性裁剪不涉及的标 N/A）+ 设计态缓解方案（具体到锁/原子/内存序/超时等）。**线程归属矩阵（O-5，条件触发：涉并发或新增/修改可变状态时必做，纯脚本/无并发工单标 N/A）**：对每个新增/修改的可变状态字段输出一张表——
  | 字段名 | 读方（线程 + 是否持锁 + 持哪把） | 写方（线程 + 是否持锁 + 持哪把） | 锁序（相对既有锁的单向性） | 并发策略（快照读 / 事件化消费 / 原子 / 单线程独占） |
  要求：同一字段**读与写不在同一把锁下**或**写方无锁**时，计划必须给出结论（快照读 / 原子 / 事件化消费）才算通过；锁序必须在既有锁序表上**单向延伸**，否则标 issue。`05_deepcheck` A7 以本矩阵为基线逐行核对
...
**TDD 测试契约表（行为变更 A 档必填；判定 `tdd.mode` 落盘 metadata.tdd）**：

> **原则（测试驱动，非实现后验证）**：每个会改变可观察行为的 A 档改动，**先定义可执行的测试契约**，步骤 4 在修改生产代码前先用该契约取得有效 RED（测试先证明能抓住问题），再做最小实现取得 GREEN。测试形态不限于单元测试——单元/静态/契约/集成/特征/主机侧协议配置/设备侧行为测试均可；但"先取得有效失败证据、再改生产代码"不能被"这是配置修改 / 改动少 / fast / patch"一句话跳过。

为每个 A 档行为变化定义测试契约表（**无法写出"修复前为什么会失败"→ 可测试性不标通过**，须先找测试挂点或把 `tdd.mode` 标 `blocked`/`device_split`）：

| 字段 | 内容 |
|---|---|
| 目标行为 | 用户或调用方可观察到什么变化 |
| 测试形态 | unit / contract / characterization / integration / device |
| 测试文件 | 计划新增或修改的测试文件（file:line 级，未定标 `?`） |
| 测试命令 | 最窄的可执行命令（不先跑全量套件） |
| RED 预期 | 具体会失败的断言 + 失败原因（修复前为何失败） |
| 非法 RED | 哪些编译/导入/环境/超时错误不能算有效失败 |
| GREEN 标准 | 同一测试在最小实现后如何通过 |
| 回归范围 | 受影响模块测试、构建和必要实机验证 |

**`tdd.mode` 判定**（落盘 `metadata.tdd.mode`）：只要修改改变可观察行为，默认 `required`；配置/接线/日志路由/协议字段/脚本接线优先 `contract`（静态/契约测试）；遗留行为不明 `characterization`（先锁现状再加目标断言）；硬件/闭源 SDK/实时性 `device_split`（主机/静态可测先测、设备验证单列）；纯文档/纯注释/生成物同步且无生产行为变化 `exempt`（须显式记理由）；测试环境/必要硬件不可用且无可信替代 `blocked`（不改写为 pass，交付验证保持 pending）。**不能因为"改动少""fast""patch"自动豁免**。判定结论与测试契约表写入 `metadata.tdd`（结构见 [SKILL.md](../SKILL.md) metadata 段）


**修复方案三档分级（反偷懒第 26 条）**：本节修复方案分 A 档（根因修复，主方案，必做）/ B 档（兜底防御，单独小节，标注"A 修复后触发概率"，用户显式确认才实施）/ C 档（后续工单，进范围外）。§3 架构设计只含 A 档改动，B 档不混入。ADR：A 档多候选走 A1/A2 对比，B 档不进 ADR。A 档跨工程（修复点不在当前工程）时 plan 明示"本工程无可实施 A 档，转交 <X> 工单"，不把 B 档提为 A 档。最小修复=只做 A 档。详见 反偷懒第 26 条。**完成后落盘 metadata**：把本节 A/B/C 三档各 1-2 条一句话摘要写入 `fix_tiers = {"A": [...], "B": [...], "C": [...]}`（字段缺失视为 null，供步骤2/4/6 核对实施范围）；**同处写 `scope_contract`**（范围契约，O-4 语义冻结，见 [references/dir_and_metadata.md](../references/dir_and_metadata.md)「scope_contract 字段族」）：`{"summary": "根因方向 + A/B/C 分档 + 验收边界 一句话指纹", "at": "运行时取系统时间", "source_step": "01_plan"}`——冻结本计划语义基线，供后续步骤检测用户输入/审查是否改变契约（字段缺失视为 null，向后兼容）

> **已受影响数据处置二选一（条件触发：根因链涉持久化坏状态）**：若根因链已证实现场存在**分叉/损坏的持久化状态**（主记录与派生工件不一致、历史数据已污染、错误状态已落盘等），本节必须明确选择二者之一：**A** 本次实现包含已受影响数据的安全修复/迁移与验证（并入 A 档）；**B** 本次只阻止复发，现有数据修复由**独立受控步骤**完成——必须写明**交接内容**（交给谁 / 哪些数据需修）、**触发条件**（何时做）与**部署后验证项**（修完怎么验证），并提示用户确认。**只防复发 ≠ 已恢复现场数据**——选 B 时交付措辞必须用"仅防复发"而非"已恢复"。本检查项与 C 档（后续工单进范围外）相容，但 C 档**不豁免**"必须写明交接"的强制要求。
   ...
   **与 §4.5 4 维度设计态的关系**：
   ...
   c.5 **方案对抗验证**（plan 阶段反推核心机制，治「自验自证」盲区）：
      ...
      - **收敛与降级**：
        ...
        - 子代理超时/失败 → 按 [references/adversarial.md](../references/adversarial.md)「子代理失败处理」重试 2 次（含 1 次换 subagent_type） → 仍失败标 `[未验证-子代理对抗失败]`，**禁止主代理补齐为代理代行**；**质疑者统一后台 spawn（`run_in_background: true`）+ `TaskOutput(task_id, block=true, timeout=...)` 阻塞等 verdict**（唯一可机械执行超时的等待方式；**禁止被动等任务通知**——挂死永无通知看门狗失守；`Agent` 工具无 timeout 参数，裸同步 spawn 无法机械执行超时）；**首次 spawn 走 10 分钟看门狗档（`TaskOutput(block=true, timeout=600000)`，`BACKGROUND_WATCHDOG_SECONDS=600`，超时判疑似断连 → `TaskStop` → 前台重来）**；前台重来/重试走 120s 档（超时先 `TaskStop` 停句柄再重试 1 次，二次仍超时走 `[未验证-子代理对抗失败]`，见 adversarial.md「显式等待 + 超时机制」段看门狗）；**质疑者 spawn 须含输出预算硬约束防 `max_output_tokens` 截断**（verdict 第一动作/禁回显/总输出≤2000 token 宽松上限勿压紧，见 adversarial.md「spawn 规格要求」第 4 条）；**整合墙钟硬截止**——对抗等待+整合阶段墙钟上限 20 分钟（`INTEGRATION_WALL_CLOCK_DEADLINE_SECONDS=1200`），3 质疑者一次性并行后台 spawn，到 deadline 未返回者先 `TaskStop` 再按已返回 verdict 整合、未返回者标 `[未验证-子代理对抗失败]`，**禁止无限等待**（见 adversarial.md「整合墙钟硬截止」段）
