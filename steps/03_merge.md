...
> **`03_plan_final.md` 必须是完整计划，不是元数据摘要**：以 `01_plan.md` 全文为基底落盘（保留 9 个正文章节），在其上叠加审查意见采纳标记（如 `[审查采纳 #1]`）与末尾「实现偏差备忘」空段。步骤 5 逆推对比、步骤 6 追溯矩阵均以本文件为计划侧输入，仅写元数据会导致功能点无法回指代码位置。
