...
11. **`requirement_deltas` 分流检查（O-4 语义冻结）**：Read `metadata.requirement_deltas`（字段缺失视为 `[]`），存在**未分流**条目（`classification` 未定，或 `needs_user_confirm` 未获 `user_confirm`，或 `needs_replan` 未重跑 plan 更新 `scope_contract`）时，**禁止定稿**——先完成分流：向用户确认 `needs_user_confirm`（确认 → 记录 `user_confirm` 后纳入定稿；拒绝 → 标 `clarification_only` 或排除）；`needs_replan` → 提示用户重跑 `/icode plan` 更新契约后再定稿；`a_now_with_evidence` → 核对计划已含对应方案（未含 → 补入）。已分流条目若改变 A 档/验收边界，同步刷新 `scope_contract.summary`。**workflow gate 升级（重大增量硬阻断）**：存在 `severity=major` 且 `needs_replan=true` 且未 `resolved` 的 delta → **禁止定稿**，必须路由回 `需求增量 → 更新范围合同 → 补齐语义决策 → 重建影响合同 → 更新验收矩阵 → 重新评审` 后才能继续（lint 阻断，见 [SKILL.md「工作流硬门禁」](../SKILL.md)）；轻量修补只允许处理不改变合同的实现缺陷
11.5. **workflow gate 定稿硬校验（L1，不通过不得进入步骤4）**：定稿前运行 `python3 tools/lint_workflow_contract.py {ICODE_OUT_DIR} --step plan --json`（语义决策 + 重大增量）与 `python3 tools/lint_workflow_contract.py {ICODE_OUT_DIR} --step code --json`（+ 身份变化影响清单）——退出码非 0 时**停止定稿流程**，按报错补齐：未解决 `semantic_decisions`（须获用户确认）、`impact_contract` 9 维未填完整（`identity_change=true` 时）、重大 `requirement_deltas` 未回流。**不得以"最保守/最安全/通常如此"代替用户选择**（见 [mcp/workflow-gate/gates.json](../mcp/workflow-gate/gates.json)）
...
- **cheap-research**（🟢*）：**仅多轮 review（>1 轮）** 时，跨轮 issue 合并汇总调 `mcp__cheap-research__summarize`（见上方「合并定稿」段；N=1 轮跳过，不评估）。其余 MCP 按路由真源判定，不默认调用。
