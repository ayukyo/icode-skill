...
4. **了解现有工程**：阅读项目中相关代码，识别现状、可复用模块、相关接口（**先于思考**，为步骤5的"现状盘点/影响面分析"提供代码依据）。**必须同步执行「现有功能覆盖度检查」**（v2.11 防重复实现机制，完整规则见 [references/necessity_check.md](../references/necessity_check.md)）：对 §3 每个新增需求点，全工程检索关键词 → Read 命中处上下文（≥20 行，消费点必须追行为链）→ 三类判定（已覆盖/部分/未覆盖）——**结论写入 §2 的「2.X 现有功能覆盖度检查」子段 + §5 预筛表「是否已覆盖」列**。本检查在 §2 现状盘点之后、§3 新增需求点之前执行
...
### 2.X 现有功能覆盖度检查（v2.11 防重复实现机制，必填）
...
## 3. 新增需求点（A/B/C 三档分列，v2.7 反偷懒第 26 条）
...
- **反推自检**（写在表后）：
  ...
  - **「是否已覆盖」列与 §2.X 交叉验证（v2.11）**：标「是（file:line）」的需求点**必须降级或删除，不得继续设计**；标「部分」的必须写明与现有实现的边界（新实现补哪一段、在哪个环节衔接）
