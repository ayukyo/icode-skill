...
6. **理解核对清单（本次新增·每轮触发）**：AI 主动列出对当前需求的理解点（5~10 条），用户逐条确认/修正。
   ...
   **提问纪律**（治「问了本该自查的问题」「顺序依赖未决」「无推荐答案」三类低质提问；不新增问答数）：
   - **前沿依赖排序**：🔴 高影响待澄清 ≥2 时按依赖关系分轮——问题 A 的答案会改变问题 B 的题意时，B 排后轮，不提前问"答案取决于还没答的问题"；每轮仍 ≤3 问，只优化顺序不增问数
   - **提问前自查门**：任何待澄清问题先自查能否 Read/Grep/历史工单/子代理查到——**查得到就不问**（查事实是 AI 职责，不是用户的）
   - **每题带推荐答案**：问用户时每题附 `➡️ 推荐答案`（可走 AskUserQuestion 推荐选项），用户只需确认或修正，无需从零构思
...
> 修复 bug 工单必填三档；纯功能开发可参考分档但非强制。A 档=根因修复（必做），B 档=兜底防御（可选），C 档=后续工单（范围外）。详见 反偷懒第 26 条。
...
### 语义决策识别（workflow gate 前置，与「待决策项」互补）

- **目的**：识别哪些待决策项会**改变外部行为**（同一输入在不同选择下行为不同）——这类决策不能靠"最保守/最安全/通常如此"代替用户选择，必须显式进入 `semantic_decisions` 合同并等用户确认（机器门禁，见 [SKILL.md「工作流硬门禁」](../SKILL.md) + [mcp/workflow-gate/gates.json](../mcp/workflow-gate/gates.json)）
- **触发识别**（命中任一即进语义决策清单）：同一输入存在 ≥2 种会改变外部行为的处理方式；结论含"待确认/未定义/需要选择/策略不唯一"；需决定允许·拒绝·合并·替换·保留·迁移；需决定存活身份·冲突优先级·兼容阈值·失败语义；当前证据只能说明缺机制不能唯一推导期望行为
- **格式**（在第5节待决策项下追加，每条一行）：`语义决策候选：<维度> — 候选A/B/C（改变外部行为点）— 倾向<X> — 待用户确认`
- **与「待决策项」区分**：普通待决策项（不改变外部行为/纯实现细节）不进 `semantic_decisions`；只有**策略选择**才进。步骤1 plan 收尾时把本节标记的语义决策候选**写入 metadata `semantic_decisions`**（`status=resolved` 需 `user_confirmed=true`）
- **零命中场景**：本需求无策略不唯一 → 写"无语义决策候选"，不豁免声明（防"未经显式判断就默认无歧义"）
...
> - **log 转出与 §7.5 一致**：log 转出时若 `log_analysis.md §7.5`（用户可读的修复前后对照）触发，`00_init.md`（**log 修复需求版**）第 4 节链路图 before/after 必须**由该图压缩而来**——节点边界、未定因果和 `[+]`/`[~]`/`[-]` 改动点不得相互矛盾；面向计划实施可保留 file:line/模块锚点，不必重复整张 Mermaid 图（init 直出版对应第 6 节；契约见 [steps/log.md](log.md)「修复前后对照契约」）
...
{按时间顺序简记每轮关键结论；可选，便于回顾本次讨论的演进。**末轮特别要求**（v2 新增）：若本次讨论中出现方案证伪/回退/方向变更（如实机发现原方案不可行、改用替代方案），末轮必须明确记录"原方案 X 不可行/已回退，改用方案 Y"（若证伪依赖外部模块/库，记录模块名 + 当时 commit，便于后续 `/icode status --verdict --dependency` 标注启用硬复活）--该末轮结论是历史检索 `unknown` 工单注入时扩读的关键（捞最终方向，详见 [../references/thinking_detail.md](../references/thinking_detail.md)「历史参考小节」），也是 `/icode status --scan` 批量识别证伪信号的扫描点；末轮缺失或未写清证伪，会导致错误方向工单继续被当正面启发注入误导新需求}
...
- **链路图与 §7.5 一致**：log 转出时若 `log_analysis.md §7.5` 已触发同图对照，`00_init.md`（log 修复需求版）第 4 节链路图 before/after 由该图压缩而来，节点边界/未定因果/`[+]`/`[~]`/`[-]` 改动点不得矛盾（init 直出版为第 6 节；详见 [steps/log.md](log.md)「修复前后对照契约」）
...
- **⚠️ 防错硬约束（先于一切判定，所有环境强制）**：图片/视频**绝不作为附件/base64 注入当前会话模型消息**——session 模型可能是纯文本（如 codex 第三方模型 glm-5.3，catalog 声明支持 image 但服务端实际只收文本），注入即报 "Model only support text input" 且图片留在上下文导致后续 turn 一启动就崩。图片分析**只能**走下方任一通道（把**文本结果**写回 §2/§3）；两通道均不可用时仅记录"图片已传、未分析，待人工查看"，**禁止**把图片回灌给模型。
- **可用性判定（双通道：MCP 工具 / 本地 CLI，config.json 三件套配齐即可用）**：
  1. **MCP 通道**：Read `~/.claude.json` 的 `mcpServers.vision-bridge` 段存在 **且** 工具可在当前会话直接调用（工具列表直接可见 `mcp__vision-bridge__analyze_media` 或代理前缀形态，或 ToolSearch 可取 schema）**且** Read `~/.claude/skills/icode/mcp/vision-bridge/config.json` 三件套（`base_url` / `api_key` / `model`）已填 → 用 MCP 工具
  2. **CLI 通道**（codex 等 MCP 工具未注入、但能执行本地命令的环境）：当前工具列表**无** `analyze_media` **且** 能执行本地命令 **且** 找到 `~/.claude/skills/icode/mcp/vision-bridge/server.py` **且** 同目录 `config.json` 三件套已填 → 用 `exec_command` 执行 `<server.py 目录>/.venv/bin/python <server.py> --analyze-media <图片路径> --prompt "<提取内容/时间点/错误提示>"`，读 stdout 文本
- **可用** → MCP 通道调 `mcp__vision-bridge__analyze_media`；CLI 通道读 stdout → 识别结果**以文本**写入 §2 现状盘点或 §3 需求点（按内容归属）
- **不可用**（两通道均不可用）→ 仅在思考块写 `vision-bridge 不可用(MCP/CLI 均不可用,<具体缺失项>)，用户图片未分析，待人工查看`，**不视为违规、不阻塞流程**
