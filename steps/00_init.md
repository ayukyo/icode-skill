...
**本节包含两部分**：① 待决策项列表（每条须标倾向 + 证据点 + 自审结果）② **可执行性预筛**（init 阶段反推核心机制，治「需求级早期可执行性盲区」，详见下方子段）
...
- **格式**（每条需求一行）：
  ...
  | 需求点（第3节编号） | 现有实现路径 | 是否可执行 | 需架构调整 |
  |----------------------|--------------|------------|------------|
  | 需求点1 | 第2节 src/foo.cpp:42 接口X 可复用 | 是 | 否 |
  | 需求点2 | 无现成路径，需新增 Y 模块 | 是 | 是（需新增 Y 模块 + 调整接口 Z） |
  | 需求点3 | 与现有 W 模块冲突 | 否（与 W 互斥，需删除 W 或重定义需求） | — |

- **必填列**：需求点（回指第3节编号）/ 现有实现路径（基于 §2 现状盘点，未知写「未知-待步骤4 调研」）/ 是否可执行（`是` / `否（原因）` / `需架构调整（说明）`）/ 需架构调整（`是` / `否` / 仅在「需架构调整」时填具体调整项）
- **反推自检**（写在表后）：
  - 每条需求都有一行，**不得跳过任何需求点**
