...
- **`/icode init` 即"新开一次需求初稿讨论"**：每次调用 `/icode init` 都**创建一个全新的 `.icode_output_N/` 目录**，**不复用**之前任何 `init_in_progress` 状态的目录、**不续聊**之前的讨论。如果用户想继续上一次讨论，就直接对话，**不要再敲 `/icode init`**。
...
3. **强制思考前置**（不可跳过，缺证据视为不合规）：先输出 `ultrathink` 触发词；再调用 `sequential-thinking` MCP，至少 4 步，每步对应一个子项：需求分解 → 现状盘点 → 影响面分析 → 待决策项识别
...
- 最新 `.icode_output_N/` 目录的 `status` 为 `init_in_progress`
