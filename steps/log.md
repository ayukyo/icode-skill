...
7. **阶段3 对抗根因分析**（复用 icode 步骤2 对抗模式：分析师+3质疑者+裁决优先级+诚实降级）：
   ...
   - **代码事实验证门（必须先于对抗）**：根因假设 H 涉及的代码行/函数，**必须先用 Read 工具实读实际代码验证**（有可索引源码时**必先调 serena**：`find_symbol` 定位函数定义 / `find_referencing_symbols` 找谁调用 / `find_implementations` 找实现，仅 Read 不算替代--Read 是文本层，serena 是语义符号层，互补非替代；serena 不可用降级 ripgrep/grep 并在思考块显式声明 `serena 不可用(<原因>)`）。**未经实际调用 serena 就标降级 = 反偷懒第 21 条违规。步骤末尾按反偷懒第 21 条 v2.6 自检门输出 `serena 调用: <工具 x N>` 或 `serena 降级: <原因>`，无记录 = 违规**（如假设"mul_overflows 在1073741824*2边界失效"，则必须 Read calc.c 的 mul_overflows 函数实际实现，逐行核对分支逻辑）。**不得仅凭日志推断代码行为**——日志显示的是现象，代码才是事实。验证通过后才进入对抗分析
