...
3. **阶段0 输入要素收敛**（最多1-2轮，不拖成长讨论）：
   ...
   - **附件分析（TB 拉取后,vision-bridge 可用则主动调,防"分析错时间点"走弯路）**：拉取到的 TB 缺陷源附件含视频/图片时,**先判定 vision-bridge 可用性**(MCP 已装 + `config.json` 三件套已配齐)。**可用→主动调**(视频先用 ffmpeg 本地提取关键帧再传图片帧省钱——见下方「TB 附件分析与 ffmpeg 抽帧」段);**不可用→仅提示附件清单**(防纯文字模型场景报错)。**强制范围**:仅在「TB 拉取后」生效,纯本地日志路径不触发(行为与改前一致)
...
6. **阶段2 日志侦察 + 现场还原**：
   ...
   - **TB 视频/图片附件研读（vision-bridge 可用则主动调,与 TB 评论研读并列）**：若 TB 缺陷源附件含视频(`*.mp4`/`*.mov`/`*.avi` 等)/图片(`*.png`/`*.jpg`/`*.jpeg` 等),**vision-bridge 可用时主动逐个调** `mcp__vision-bridge__analyze_media`(视频先用 ffmpeg 本地提取关键帧省钱——见「TB 附件分析与 ffmpeg 抽帧」段);或复用 §1 已落盘的附件分析结果但须独立回捞证据点。**视频/图片里提取的时间点(界面时钟)+ 现象描述 + 用户操作 + APP 状态必须回捞进「现场时间线」表**并标注来源「TB附件:<文件名> <时间点>」,与 TB 评论来源并列、交叉验证。**完整性自检**(vision-bridge 可用时适用):「已分析附件数 == meta.json `files[]` 中视频/图片附件数」(漏个视为不合规,反偷懒第 23 条);复用场景下对比 `*_meta.prev.json` 识别新增附件;vision-bridge 不可用时仅记录附件清单不适用本自检
...
8. **阶段4 报告生成 + 衔接步骤1**：
   - 用 Write 写 `{ICODE_OUT_DIR}/log_analysis.md`，骨架：
     ...
     **一句话定性**：<症状触发→根因链路→结果>，置信度 <高/中/低>，修复方向 <具体方案>
     ...
     ## 1. 输入要素 —— 五要素（日志目录/问题描述/问题时间点/前序场景状态链/TB 附件清单与附件分析结果），标注推断来源（TB 附件清单：视频/图片的文件名+大小+类型，**无条件记录**（无视频/图片时该小节不存在）；「附件分析结果」vision-bridge 可用时由「TB 附件分析与 ffmpeg 抽帧」段写入,含时间点+现象+关键帧路径；不可用时仅记"vision-bridge 不可用"降级声明于附件清单小节末尾）
     ## 2. 基线检查
...
## TB 附件分析与 ffmpeg 抽帧（实战补强，vision-bridge 可用则主动调，防"分析错时间点"走弯路）

**核心原则**：TB 缺陷源附件含视频/图片时，**先判定 vision-bridge 可用性**(MCP 已装 + `config.json` 三件套已配齐)。**可用→主动调**（视频先用 ffmpeg 本地提取关键帧省钱）；**不可用→仅提示附件清单**（防纯文字模型场景报错）。
...
1. **枚举附件**：Read `<ID>_meta.json` 的 `files[]` 字段筛出视频(`*.mp4`/`*.mov`/`*.avi` 等)/图片(`*.png`/`*.jpg`/`*.jpeg` 等)，**无条件记录附件清单**(文件名+大小+类型)到 `log_analysis.md §1`「附件清单」小节（可空章节，无视频/图片时该小节不存在）
...
**分析流程**（vision-bridge 可用时自动走，在阶段0 内、TB 拉取后、强制思考之前）：
...
1. **视频抽关键帧**（ffmpeg 本地免费处理，省钱）：
   ...
   - **ffmpeg 可用**：对每个视频附件用 ffmpeg 提取场景切换关键帧
     ...
     ffmpeg -i <video> -vf "select='gt(scene,0.1)',scale=1280:-1" -vsync vfr -q:v 2 -frame_pts 1 \
       "{ICODE_OUT_DIR}/tb_source/<ID>/frames_<video_basename>_%03d.jpg"
     ```

   - **ffmpeg 不可用**：降级为直接传视频给 vision-bridge（需提示用户「ffmpeg 不可用,直接分析视频可能消耗 API 额度」），或仅分析图片附件
2. **枚举关键帧**：`ls {ICODE_OUT_DIR}/tb_source/<ID>/frames_*.jpg` 列出所有关键帧文件
...
> **与阶段2「TB 评论研读」的关系**：评论研读是逐条研读 `comments[].content.comment` 文本（**强制，必逐条不漏**）；**附件研读是逐个研读视频/图片二进制附件**——vision-bridge 可用时与评论并列主动调(视频经 ffmpeg 抽帧后传图片)；两者并列，缺一不可。
