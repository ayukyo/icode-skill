...
5. **阶段1 基线检查**（grep 日志之前必做，借鉴 baseline-checklist）：
   ...
   - **代码库归属后深查指引（v2.6 强制，防"只 grep 当前仓漏 thirdparty"）**：归属判定后**强制扩展 grep/serena 检索到以下位置**（关联反偷懒第 24 条姐妹工程检索）：
     ...
     - **深查方法**：提取独特字符串（如 `<mutex_name>` / `<error_msg>`）→ serena search_for_pattern + git grep 所有候选位置 → 定位治本代码
...
6. **阶段2 日志侦察 + 现场还原**：
   - **侦察范围清单（v2.6 强制，防"只盯 *.log 漏掉治本物证"）**：阶段2 开始**先 `ls -laR` 整个数据源目录**，按以下 4 类逐一盘点（每类至少 ls/head 看一次，不得只扫 *.log）：
   ...
   - **§3.1 前序场景状态链（v2.3 实战补强，**`**P0**`**）**——问题时刻往前 30-60 分钟的状态/任务/模式切换表，**通用抽象**：
     ...
     - **扫描字段**（**按领域填，不限于机器人**）：
       ...
       - 状态值语义对齐（v2.4 新增，跨模块）：问题时刻前出现的外部模块状态字段，其在本模块被解读的语义是否与外部模块原生语义一致（与 §2.2 跨模块枚举对照表交叉验证）
...
7. **阶段3 对抗根因分析**（复用 icode 步骤2 对抗模式：分析师+3质疑者+裁决优先级+诚实降级）：
   ...
   - **根因树 3 层假设链（v2.6 强制，防"停在症状层"）**：分析师提假设时**必须输出 3 层假设链**，不得只提单层症状归因（按 [templates/root_cause_tree_template.md](../templates/root_cause_tree_template.md) 模板填写）：
   ...
   - **上游假说继承/接力模式（v2.6 强制，防"忽略早前假说"）**：分析师提假设前**必须先扫 TB 评论 + 历史工单**找含"我猜测/可能是/会不会是"的假说性表述，每个继承的假说输出三段式：
     1. **继承了什么**（引用原文 + 提出人 + 时间）
     2. **否决/部分否决的依据**（实证，不得悄悄忽略）
     3. **在此基础上补了什么**（新证据或新假设）
     - 即使上游假说未中，也须显式记录"已评估但否决，依据 X"--诱导方向的假说不得被悄悄丢弃
   - **代码事实验证门（必须先于对抗）**：根因假设 H 涉及的代码行/函数，**必须先用 Read 工具实读实际代码验证**（有可索引源码时**必先调 serena**：`find_symbol` 定位函数定义 / `find_referencing_symbols` 找谁调用 / `find_implementations` 找实现，仅 Read 不算替代--Read 是文本层，serena 是语义符号层，互补非替代；serena 不可用降级 ripgrep/grep 并显式声明 `serena 不可用(<原因>)`）。**未经实际调用 serena 就标降级 = 反偷懒第 21 条违规。步骤末尾按反偷懒第 21 条 v2.6 自检门输出 `serena 调用: <工具 x N>` 或 `serena 降级: <原因>`，无记录 = 违规**（如假设"mul_overflows 在1073741824*2边界失效"，则必须 Read calc.c 的 mul_overflows 函数实际实现，逐行核对分支逻辑）。**不得仅凭日志推断代码行为**——日志显示的是现象，代码才是事实。验证通过后才进入对抗分析
   - **对抗分析**：完整对抗模式（3质疑者/subagent_type=general-purpose+schema 强制结构化/裁决优先级/诚实降级/证据回指/子代理失败处理）——**必须先 Read [references/adversarial.md](../references/adversarial.md) 完整内容**（不得凭概述/记忆执行）。**必须独立 spawn 3 个质疑者子代理**（证据质疑者/替代解释者/充分性质疑者各一，不得合并 spawn，少任一视为不合规；**禁用 Explore**，用 general-purpose + schema 强制结构化输出防截断）。本步骤分析对象 = 根因假设 H（已通过代码事实验证），证据指针指向日志行（节点+时间+原文）和代码行（file:line），输入契约喂质疑者「日志目录路径 + 现场时间线 + 假设 H + 证据指针 E + 代码路径 + **段零文档清单（步骤 4「段零文档与姐妹工程检查点」盘点结果）+ 姐妹工程搜索结果（§2.0 候选代码库路径列表）+ **§2.2 跨模块枚举对照表（如已生成）****」，让质疑者也能检查"根因假设是否漏了关联工程的代码路径 / 是否与文档描述的设计意图矛盾 / **上游传来的状态值是否被本模块按错的语义解读（与 §2.2 对照表交叉核验）**"。**（v2.4 增补）上游语义追问**：证据质疑者必须独立追问「上游 X 模块传过来的状态值 N，在 X 原生语义下到底是『终局态』还是『过渡态』？本模块消费者有没有按错的语义解读？」——若未追问即使其他维度齐全亦视为对抗不完整（漏掉最常见的「语义碰撞」型根因）。
...
子代理失败时按 adversarial.md「子代理失败处理」重试1次→仍失败诚实降级为 `[未验证-子代理对抗失败]`，**绝不自演裁决**。**环境无 spawn 工具时**（结构性不可用，非 transient 失败）按 adversarial.md「环境无 spawn 工具场景」处理——不重试，诚实降级为 `[未验证-环境无spawn工具]`，产物文件强制明示（§0 警告行 + §6 子章节标题），主代理代行三视角独立判断但不得自演一致假共识
...
8. **阶段4 报告生成 + 衔接步骤1**：
   - 用 Write 写 `{ICODE_OUT_DIR}/log_analysis.md`，骨架：
     ...
     **一句话定性**：<症状触发→根因链路→结果>，置信度 <高/中/低>，修复方向 <具体方案>
     ...
     - §2.2 跨模块枚举对照表（v2.4 新增，防"语义碰撞"类根因）—— 当数据流跨越 SDK / 其他进程 / 共享库边界时，列出每个跨边界状态字段在两侧的枚举定义并标注「同名数值是否同义 / 过渡态 vs 终局态」；不涉及时该小节可空
     ## 3. 现场还原
