...
> **修复前后对照契约（§7.5，触发判定 + 产物要求 + 最终答复）**：让复杂故障的一次交付同时回答"现场链路在哪断 / 哪些事实已证实、哪些只是待验证因果 / 修复新增或改变了哪个控制点 / 修复后怎样阻断原故障又保留原有安全保护"。**不是增加装饰图，也不改变根因证据等级**（真源：[references/log-repair-visual-comparison-spec.md](../references/log-repair-visual-comparison-spec.md)）：
