...
2. **历史检索复用**（强制思考之前，全局索引存在时必须执行，详见 SKILL.md「历史检索复用」段）：
   - **`--debug` 分支（独立孪生对照 → 跳过源1·历史工单检索）**：debug 工单是**独立孪生**——它应**被参考**（产物供正常工单并列对照研读，见 [references/debug_mode.md](../references/debug_mode.md)），**不去参考历史工单**。原因：历史检索读 `index.json` 注入的是**历史正式工单**的根因结论（debug 工单不入索引），debug 分析若参考它们，会被既有结论带偏、无法独立形成自己的思考——正常工单与 debug 孪生两份对照都会收敛到同一历史结论，对照价值（视角差异/假说强度/证据缺漏）尽失。因此 debug 模式**整段跳过**下方源1·历史工单检索（两段式检索 / verdict 分流注入 / 结论级时效校验 / 重复模式检测——后两者依赖源1 命中，随源1 一起跳过），**保留**源2·段零工程文档检索（代码事实快照，非"别人结论"，帮助理解当前代码）+ limit 红线检查点（项目级约定，debug 同样遵守）。**debug 域内同 TB 单旧 debug 孪生复用不受影响**——那是"参考自己"（步骤1 debug 变体），非"参考别人"。**跳过后在思考块「历史参考」标注**"debug 模式跳过历史工单检索（独立孪生对照），无历史工单参考，仅注入段零工程文档"（步骤4 历史参考小节）
...
4. **思考分级（L2：sequential-thinking）**（按 [references/mcp_per_step.md](../references/mcp_per_step.md)「通用前置·分级思考」段执行）：本步骤为 L2，调用 sequential-thinking（3~5 步）。思考职责 = 症状分解 → 基线预判 → 链路假设 → 验证策略。**若步骤2有历史参考，在此处「历史参考」小节记录命中工单 id 与根因要点，作为思考输入**（**debug 模式**：步骤2 已跳过历史工单检索——独立孪生对照，此处标注"debug 模式跳过历史工单检索，无历史工单参考，仅注入段零工程文档"，不伪造历史参考）
...
8. **阶段4 报告生成 + 衔接步骤1**：
   ...
   > **机制证据 vs 业务策略结论（workflow gate 前置，防"替用户选策略"）**：日志分析结论必须**显式区分两类**：
   > - **机制证据**：代码/日志/历史可实证的事实——如"存在两个根因候选""策略未定义""存在多个可行选项""缺少 XX 机制"。**机制证据可以由分析独立给出**，写进核心结论/根因分析
   > - **业务策略结论**：需要用户拍板的**策略选择**——如"该用拒绝还是保留多实体""冲突时以谁为存活身份""失败语义应该怎样"。**证据可以证明机制缺口，但不能代替用户选择业务策略**——一旦机制证据指向"策略未定义/多选项"，必须把该策略选择列为 `semantic_decisions` 候选（结构见 [SKILL.md「工作流硬门禁」](../SKILL.md) + [mcp/workflow-gate/gates.json](../mcp/workflow-gate/gates.json)），交 plan/用户确认；**禁止在 log 阶段以"最保守/最安全/通常如此"直接选定**。若本轮只做诊断（策略未定），在 `00_init.md`/metadata 标 `diagnosis_only=true`（"仅诊断"，允许结束但不进实现）；策略已由用户明确选择且已写入 `semantic_decisions` 时，log 可直接给出建议修法（状态 resolved）
...
> 思考分级完整规则**按 [references/mcp_per_step.md](../references/mcp_per_step.md)「通用前置·分级思考」段执行**（本步骤 L2；三件套 Read 照常），本步骤思考职责见上方步骤4。
