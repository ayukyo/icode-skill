...
3. **阶段0 输入要素收敛**（最多1-2轮，不拖成长讨论）：
   ...
   - **前序场景状态链（v2.3 实战补强，第 4 要素）**：用户提"问题时刻前 30-60 分钟发生过什么 / mission 切换 / 状态迁移"时主动探查；不知则问"问题发生前这段系统在做什么"。**通用抽象**：不限于机器人 任务调度器；任何"任务/状态/模式切换"都属此类
   - 收敛出三要素（+ 姊妹工程清单 + 前序场景），写入 `log_analysis.md` 的「输入要素」章节
...
8. **阶段4 报告生成 + 衔接步骤1**：
   - 用 Write 写 `{ICODE_OUT_DIR}/log_analysis.md`，骨架：
     ...
     **一句话定性**：<症状触发→根因链路→结果>，置信度 <高/中/低>，修复方向 <具体方案>
     ...
     ## 1. 输入要素 —— 四要素（日志目录/问题描述/问题时间点/前序场景状态链），标注推断来源
     ## 2. 基线检查
