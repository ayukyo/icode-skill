...
**用户语义变更检测（O-4 语义冻结，写 requirement_deltas）**：读 `metadata.scope_contract`（缺失视为 null＝未冻结，跳过本检测，向后兼容旧工单）；若**用户本次输入**改变了冻结契约的语义——状态身份或生命周期、允许/拒绝条件、持久化一致性或回滚承诺、验收条件/调用方语义/真实环境验证场景——不得静默按新语义继续审查，须**先分类写入 metadata `requirement_deltas`**（追加，字段缺失视为 `[]`）：`clarification_only`（仅澄清不改变实现，可继续）/ `a_now_with_evidence`（改变 A 档但已有直接证据，记 impact 后按 A 档处理）/ `needs_user_confirm`（需用户确认，未确认前**停止自动串联**等待确认）/ `needs_replan`（需回到 plan/review 重新定稿，**先停本步骤提示用户重跑 plan**）。**delta 未分流前不得继续扩大代码设计或验收矩阵**（冻结点）。每条含 `{at, user_input_summary, changed_aspect, classification, impact, user_confirm}`，见 [references/dir_and_metadata.md](../references/dir_and_metadata.md)「requirement_deltas 字段族」。**workflow gate 联动**：用户本次输入若改变**存活身份/依赖迁移/失败语义/持久化·事务·回滚·恢复/新增跨组件·跨仓·外部消费者影响/验证边界扩大**——在 `requirement_deltas` 记为 `severity=major, needs_replan=true`（重大增量，lint 会阻断并路由回 plan，见 [SKILL.md「工作流硬门禁」](../SKILL.md)）。

**影响清单独立审查（workflow gate·身份变化，防漏消费者/引用/恢复路径）**：读 `metadata.impact_contract`（缺失视为 null＝未声明身份变化，向后兼容）；若 `identity_change=true`，审查必须独立核对 9 维影响清单**每项是否被 plan 覆盖且证据充分**——重点独立搜索计划**未提及**的消费者/引用/恢复路径：①聚合/批量选择（全选/计数/批量）②依赖记录的身份引用（持久化外键/索引）③运行时缓存/注册表/索引 ④异步观察者/延迟刷新 ⑤重启/重放/幂等重试 ⑥外部投影/下游消费者 ⑦中途失败回滚。发现遗漏 → 记 issue，建议回到 plan 补齐 `impact_contract`（9 维每项必答 + `completeness=complete`），**遗漏未补前不得进入编码**（lint `--step code` 阻断）。
...
3. **分步续跑检测**（必须在强制思考之前，决定本轮是首轮还是续跑，并判定 fast 场景）：
   ...
   - 否则**首轮初始化**（status 为 `plan_done`/`review_done`/其他非 in_progress 态）：`status=review_done` 表示上一轮审查已收敛终止，再调 `/icode review` 视为**重新审查**——`clean_rounds = 0`, `total_rounds = 1`, `extended_rounds = 0`, `max_rounds` 由参数决定（`param_max_rounds` 非空用 `param_max_rounds`，否则默认 3），`absolute_cap = max(10, max_rounds × 2)`，设 `status = review_in_progress`（**控制面接线**：经 `python3 tools/icode_control.py transition --dir {ICODE_OUT_DIR} --to review_in_progress`，进行态流转仅做状态机合法性判定，见 [references/control_plane.md](../references/control_plane.md)），将 `max_rounds` / `absolute_cap` / `extended_rounds` 写入 metadata。**场景一 `FAST_LOCKED=true` 时强制 `max_rounds=1`（覆盖上述决策）**。**重新审查会覆盖旧 `review_round_*.json` 与 `02_review.md`**——若用户想在中断处续跑，应确保 status 是 `review_in_progress`（中断态）而非 `review_done`（终止态）
4. **思考分级（L2：sequential-thinking）**（按 [references/mcp_per_step.md](../references/mcp_per_step.md)「通用前置·分级思考」段执行）：本步骤为 L2，调用 sequential-thinking（3~5 步）；基于上述第3步「分步续跑检测」的判定结果选择思考路径：
   - **首轮**（`total_rounds == 1`）思考职责：需求分解 → 独立方案构思 → 对比要点预判
   - **续跑**（`total_rounds > 1`）思考职责：回顾历史轮次问题 → 增量审查范围界定 → 跨章节影响预判
...
**TDD 测试契约审查（行为变更 A 档，审查计划 §4.5 的 TDD 测试契约表 + `metadata.tdd`）**：
1. 测试是否验证**外部行为**（用户/调用方可观察），而非复述实现代码——过度绑定私有函数/日志文案/源码排版 = issue
2. 测试在**生产修改前是否预期失败**（RED 预期明确、能写出"修复前为何失败"；写不出 → 可测试性不标通过）
3. RED 是否可与**环境错误**明确区分（编译/导入/权限/超时不属于有效 RED）
4. **测试首次即通过**时是否有停止/必要性复核分支（不得以"测试通过"作为继续写代码的理由）
5. 是否覆盖正常、边界、异常**以及本次缺陷复现场景**
6. `tdd.mode` 判定是否合理（不能因改动少/fast/patch 自动 exempt；`exempt` 须有理由）
...
5. **终止后更新 metadata**：`status = review_done`（**控制面接线**：经 `python3 tools/icode_control.py transition --dir {ICODE_OUT_DIR} --to review_done`——完成态门禁点，transition 会运行三 gate linter（`--strict`），任一失败状态不前移；下方第 7 步的 `--step review` 专项校验保持并行执行，两者互补），`completed_steps` 追加 `"2"`，保留 `extended_rounds` / `unresolved_issues_at_cap` / `pending_verification` 字段供后续步骤参考
