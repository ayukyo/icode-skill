...
- **重启/重连同一性（deploy 意图含重启设备/reboot 时必做）**：**重启前先记录设备指纹**作身份基线——`hostname` + 各网卡 MAC（`cat /sys/class/net/*/address`，排除 `lo`），有序列号/产品 ID 更好；**重启后不固定 sleep**（设备启动时长不定，常见 30s~1min）——**轮询重连**：每 5~10s 尝试 SSH 连接，直到可用或超时（上限约 120s，超时 → 报告「无法重连」并停止，不静默继续）；**连上后立即核对指纹**——hostname/MAC/序列号与重启前一致 → 确认同一台设备，才继续「轮询监听」；**不一致（连到非目标设备）→ 输出提示**「⚠️ 重连到非目标设备（MAC/序列号不符），继续轮询重连中」，**不停止、不落盘**——用户一般会在超时窗口内切换连接，此时继续重试即可（多设备同网段时 DHCP 重新分配/同型号设备可能连错，属预期过程）；**持续到超时仍未连到目标设备 → 才停止并落盘记录**「无法重连到目标设备」报告用户，**禁止把非目标设备日志当目标设备分析**（IP 会变，**不得用 IP 判断同一性**，必须是 MAC/序列号等硬件级指纹）
