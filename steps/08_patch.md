...
**命令**: `/icode patch [问题描述或新需求...]`（可选 `--listen` → 阶段 4「1.5 实机部署验证」自动监听：连设备部署 + 持续轮询 + 实时链路分析，告知触发动作后立即监听、用户随时操作被捕获；无 flag 默认跳过 1.5。需要用户显式触发节奏的纯验证统一使用 `/icode verify --test <target>`）
...
3. 确定本次 `N`（**双源取大，防编号冲突**）：
   ...
   - **未完成段处理**：先读 `metadata.patch_phase`（可选字段，枚举 `plan_done` / `implementing` / `listening` / `awaiting_user_action` / `finalized`，缺失视为 null）——存在且为 `listening` / `awaiting_user_action` **且本次调用为 `/icode patch --listen`**（续跑监听）→ 输出提示"检测到进行中的 Patch {最大编号}（patch_phase=<...>），本次**续跑**回到监听/触发等待，不新开段"；为 `implementing` → 续跑回到对应实施阶段；**其余情况**（`plan_done` / `finalized` / 缺失，或调用意图与 phase 不匹配——如非 `--listen` 的新请求）且最大段编号 > `patch_count`（存在未完成的 Patch 段，未落 patch_history 记录）→ 输出提示"检测到未完成的 Patch {最大编号}（未落 patch_history），本次开启 Patch {N}"，并在该未完成段末尾追加一行 `> **未完成**：被 Patch {N} 取代（未走强制操作，无 patch_history 记录）`——段编号永不冲突，未完成段有明确收尾标注
...
**思考分级（L2：sequential-thinking）**（按 [references/mcp_per_step.md](../references/mcp_per_step.md)「通用前置·分级思考」段执行）：本步骤为 L2，调用 sequential-thinking（3~5 步）。思考职责 = 现状重审要点 → 修改点清单 → 影响面预判 → 复检策略

**思考产出进 `08_patch.md` 对应段，不写工程产物**。

**思考自检门**（阶段 2 计划 / 阶段 4 复检各一次）：每次思考分级（L2）执行后，输出确认行 `思考分级: sequential-thinking <N> 步` 或 `思考分级: 降级文字块（<原因>）`——无确认行 = 未执行思考 = 不合规（防"规则有但执行跳过"）。
...
- 环境无工具：`⚠️ 降级 [未验证-环境无spawn工具]` + 主代理文字块自演质疑列点
...
4. **Patch N 段追加批注**（不论结果）：在「3. 增量计划」末尾加一行 `### 修复方案对抗结果`：
   ...
   - 降级：`⚠️ 降级 [未验证-对抗不通过]（质疑者 X: refuted — <一句理由>）` + 主张由用户决定是否继续
5. **环境无 spawn 工具降级**（参考 adversarial.md 同机制）：ToolSearch 取不到 Agent → 标 `[未验证-环境无spawn工具]`（**与 refuted 区分**：env-only 不算方案有问题，是工具不具备）；主代理在这种降级下需**额外说明**已用文字块自演质疑（列出至少 3 个反向质疑点 + 自答），作为降级二级兜底
...
> **代码修改型 patch 的 TDD 轻量门（RED→GREEN，与 [04_code.md](04_code.md)「TDD 准入门」一致但轻量）**：本 Patch 修改生产代码（非分析验证型豁免）时，**先取得有效 RED 再改生产代码**：
> 1. 阶段 2 增量计划写「最小测试契约」（目标行为 / 测试命令 / RED 预期 / GREEN 标准——可从主流程 `metadata.tdd` 继承同一契约，但**每个代码修改型 Patch N 独立记录自己的 RED/GREEN 证据，不借用主流程旧测试结果**）
> 2. 阶段 3 实施顺序 = **测试 Edit**（只加/改测试及其最小测试基础设施）→ RED 证据（`failure_class=expected_assertion` 才有效，写入 Patch N 段）→ **生产代码 Edit** → GREEN（重跑同一命令退出码 0）
> 3. 测试首次即通过 → **停止并重审**（行为可能已存在 / 测试未覆盖目标，不得以"测试通过"作为继续写代码的理由）
> 4. 分析验证型豁免（无代码修改）不受本门约束，但须保持显式声明"分析验证型，无代码修改"；**部署后验证发现型**不能直接改生产代码，先把现场现象下沉为最小可执行契约，确实只能实机复现时用 `tdd.mode=device_split`
> 5. RED/GREEN 证据写入 Patch N 段「实施/验证」小节 + `metadata.tdd.red/green`（追加本次 patch 记录）
...
**思考分级（L2：sequential-thinking）**：思考职责 = 列本次修改点 → 逐点预判破坏面 → 定复检断言（**分析验证型分支**：= 列分析结论 → 预判结论薄弱点 → 定反向质疑策略）
...
- **触发**：仅当 `/icode patch --listen` 调用时执行；无 flag → 跳过 1.5（保持可选，不阻断）。需要显式触发与短轮确认时改用 `/icode verify --test <target>`
- **读配置**：先按 [dir_and_metadata.md](../references/dir_and_metadata.md)「resolve_project_id」算法算出 `project_id`（git 仓库内 = `basename(git rev-parse --show-toplevel)`，**须含 F1 worktree 归一化**：当前 checkout 的 `.git` 为普通文件（`gitdir:` 开头）= worktree 成员时，project_id 归一到**主仓根**（`git worktree list --porcelain` 首行，勿用 `$2` 字段防含空格截断，见 [dir_and_metadata.md](../references/dir_and_metadata.md)「project_id 与 branch 语义」）——worktree 工单不做 F1 会把 project_id 解析成 worktree 根名（如 `<repo>-wt-<ticket-slug>`）→ device_config / limit 主存读不到；**不得用 `ticket_id` 前缀或外层目录别名凑文件名**）→ 拼路径 `~/.claude/icode_data/device_config/<project_id>.json` 再 Read（**单文件多连接**，模板 [templates/device_config.json.template](../templates/device_config.json.template)）。**读后校验**：文件内 `project_id` 字段必须等于当前解析值，不一致 → 提示"配置文件内 project_id 与当前工程不符，改后重试"。文件不存在或 `deploy_enabled=false` → **不静默跳过**，先做**缺失诊断**：输出 `project_id = <实际解析值>`（worktree 工单此处已是主仓根，勿再按 cwd git 根核名）+ `ls ~/.claude/icode_data/device_config/` 已有文件——若存在名称近似的配置文件 → 提示"疑似文件名与 project_id 不匹配：将配置改名为 `<project_id>.json`（或建软链）并同步文件内 `project_id` 字段后重试"；无任何文件 → 提示"⚠️ `--listen` 显式要求实机验证，需在 `~/.claude/icode_data/device_config/<project_id>.json` 配置连接（或去掉 flag）"
...
- **产物-改动核对（部署前必做，防部署错二进制）**：部署前先推导"本次改动文件 → 受影响二进制"并与部署目标核对——`git diff --name-only` / metadata `code_files` 取本次改动文件 → 按目录归属 / 构建产物映射（构建目标名 / 子仓名 / 文件名前缀）推导应为哪个二进制 → 与用户表述的 `<target>` / `connections[].deploy` 意图比对。**不一致 → 输出告警**"⚠️ 您说部署 X，但改动文件在 Y（子仓）→ 部署产物应为 Z，确认后继续"，确认后再部署；**推导不出唯一产物 → 先问用户确认部署目标，不猜**。
...
- **设备端命令规范（部署/监听的所有设备端 shell 统一适用）**：设备端工具集受限（如 BusyBox），**禁止 `grep -P`/PCRE**（本机 `grep -oP` 惯用法直接搬到设备端会失败）——一律用基础 BRE（`grep -o`/`grep -E` + 正则转义）；进程/产物匹配统一用 **basename**（设备进程路径常含 `scripts/../bin/` 之类相对段，完整路径 grep 会失配，`grep <进程名>` 才有效）。AI 组装远端命令时自动降级
...
- **智能监听分析（四步；每轮对增量执行前三步，三态判定仅在终止条件命中后执行一次）**：
  ...
  2. **过滤定位（每轮）**：即使增量返回**多子目录多文件**（如 `<node_a>/daily/`、`<node_b>/boot/` 等几十个），**不逐文件全读**——先对增量文件按特征 `grep` 命中片段，只读命中内容，其余跳过；无任何命中 → 疑点（**可能「未触发」或「未走到」，不预设失败**），累计空转轮次，供 `--listen` 空转超时（8~10 轮）判断
  ...
  4. **成功判定（三态；仅终止条件命中后执行一次）**：①**修复生效**——特征打印出现 + 链路关键点正常 + **全链路/相邻 node 无新增 error**（回归信号，防"修 A 引入 B"假成功）→ 判定「修复生效 / 链路通」，记录 file:line 证据，**显式告知用户**"✅ 修复生效 / 链路通"；②**未触发**——特征缺失 + 无链路活动 + 外部线索（连接状态 / 请求源 / 用户是否已执行触发动作）表明**触发条件未发生**（含 `--listen` 空转超时用户未操作）→ **不进入闭环**，显式标注「未触发」，告知用户需执行触发动作，等待后再监听（写 `patch_phase=awaiting_user_action`）；③**修复失败**——特征缺失 + 链路有活动 / 有 error 可定位 → 进入「闭环」。**判定前先做「特征可见性核查」（防"特征被日志级别/模块过滤挡掉"误判成未触发/失败——目标特征日志级别低于设备日志级别 / 模块级 tag 过滤时全程不可见，属监听盲区）**：目标特征持续缺失但**链路有活动**（有请求进来 / 有处理日志 / 有 error）→ 先查目标特征所属日志级别（从 diff 提取，区分低/高日志级别特征）与设备当前日志级别（及模块级 tag 过滤，tag 可被单独过滤）——**目标级别低于设备级别 → 属「特征不可见」而非未触发/失败**，输出"⚠️ 特征 `xxx` 级别过低，可能被设备日志级别过滤，只能靠行为证据闭合"并转「行为证据闭合」；级别匹配或无法确认 → 才走 ②③ 判据。**判定记录必须标注证据类型（双通道）**：**日志特征证据**（特征打印 / 链路日志）vs **行为证据**（上报值 / 返回码 / 字节数 / 状态位 / 用户实机观察），行为证据须标注强度（直接观测 > 排除法推断），特征不可见时显式声明"靠行为证据闭合"。**未触发 vs 失败的判据 = 触发条件是否已发生**（有外部请求进来过？连接是否建立？），而非"有没有特征"；无法确认触发是否发生 → 归「未触发」，先问用户，勿进闭环。日志只证明"代码执行到打印点"，不 100% 证明逻辑全对——`--listen` 定位是快速看修复是否生效、链路是否断，**最终验收仍以用户观察实机行为为准**（防把"链路走通"当"功能验收"）
...
- **死胡同检测（防在错误方向上空转）**：若**连续 2 轮失败点相同**（同一特征始终缺失 / 同一 error 重现）→ 判定为**方向或部署层问题**而非"改没改够"，**跳出增量修复**，先做三层深查：① **部署核查**——deploy 是否真的生效（exit code / 产物 mtime 是否更新 / 目标进程是否重启加载新产物；"改对了但没生效"最常见原因是部署层失败）② **方向复盘**——重新审视 bug 根因判断（链路是否理解错 / node 映射是否错 / 修复方向是否偏）③ **特征校验**——grep 特征是否真由本次代码产生（tag 拼写 / 是否有旧进程在写 / **是否被设备日志级别或模块级 tag 过滤**——目标特征日志级别低于设备级别时全程不可见，须转「行为证据闭合」）。深查修正方向后**重置 3 轮计数**再走闭环；深查无果（方向无法确认）→ 停止，报告用户建议 `/icode plan` 重审
...
- **主工单验收闭环（旁路修复后强制回主验收，防主目标被搁置）**：`--listen` 中若本次 Patch 修的是**旁路问题**（非主工单原始目标，如主目标之外顺带发现/被外部事件带出的问题），修复后**必须回到主工单验收清单逐项核验并记录**——主验收清单 = `00_init.md` §7 / §5「4 维度验证清单」+ metadata `requirement`（原始需求）逐条；主验收**未全部通过前，本 Patch 不得标 `done`、工单不得标 `completed`**。`patch_count`/`patch_history` 记录旁路 patch，但**主验证单独追踪**：在 Patch N 段追加「主验收核验」小节，逐项给 通过/未通过 证据；未通过项 → 进闭环修复（不抹掉主验收清单，修完回到本小节更新状态）。**收尾强制**：强制操作第 2 条写 `06_audit.md` 补丁记录段时，主验收未闭环 → 同段追加告警「⚠️ 主工单验收未闭环：<未通过项>」，供 audit 维度 7「原始需求收敛」复检拦截（[06_audit.md](06_audit.md) 已强制逐条核对原始需求是否满足）。
...
> **patch 是全场景开放步骤**——测试发现的问题可能涉及 UI 截图/视频证据、前端行为、第三方库、历史工单记忆等，**不预设任何 MCP 不适用**：除 sequential-thinking（L2 必用）外，其余全部 🟢*（满足强证据场景才必调，不满足自动降 ⚪ 无需声明）。各 MCP 的强证据场景见下方表。

| MCP | 推荐级别 | 用途（强证据场景） |
|-----|----------|------|
| sequential-thinking | 🟢 | 思考分级 L2（执行前/阶段4 复检两次，sequential-thinking 3~5 步） |
...
- **`patch.listen_log_summary`**（tool=summarize）：`--listen` 每轮监听**只统计本轮增量窗口**（增量候选日志字节数 = `len(增量文本.encode("utf-8"))`）≥ `long_text_threshold_bytes` → `eligible=true` 必须 called/cache_hit/degraded_after_attempt；< 阈值 → 主代理直接读，`eligible=false, skipped_not_eligible`（evidence 含 listen_mode/incremental_bytes/threshold）。**禁止把历史旧日志反复计入触发**。
