...
**对外表达契约**：本简报的对外组织与表述（开头结论/现象与影响、归因措辞分级、实现主体/运行载体/下游表现/观测载体四角色拆分、「日志/观测说明」语义拆分、修复与验证状态显式标注、首屏质量门）**统一按 [references/external_brief_contract.md](../references/external_brief_contract.md) 执行**；本段只保留 readme 特有差异（下述交付报告分工、核心代码条件、`delivery_verdict` 状态来源）。
