...
**落点约束（worktree 工单）**：读 metadata `active_checkout`（缺失按 [references/worktree_isolation.md §3.7](../references/worktree_isolation.md) 用 `worktree_path` 推导）非 null → 须先 `cd {active_checkout.path}` 校验/读取代码。普通 worktree 工单的产物在该 checkout；**reopen 工单**从 `.icode_output/.active_ticket.json` 解析 `control_root`，本步报告仍写 `artifact_root=control_root`，不得在新 checkout 另建同 ticket 目录。
...
**触发**：读 metadata `worktree_path` **或** `active_checkout`（按 [references/worktree_isolation.md §3.7](../references/worktree_isolation.md) 推导）**任一非 null**（工单在 worktree 内完成/曾迁移）→ **仅交付报告**追加「worktree 状态」段（位置：交付报告「代码仓库」行之后）。两字段均 null（原地工单）→ 不生成本段。
...
**简报不出现 worktree 状态段**：跨领域简报（`_brief.md`）默认**不**含「worktree 状态」段——简报面向其它模块同事，证据叙事关注"现象/根因/修复/验证"，**不写 worktree 清理命令 / 回流操作 / 绝对路径**（那些是给本仓开发者的收尾信息）；worktree 相关只在本仓内部文档（交付报告）体现。
...
**TB/日志源简报**：本节必须是**原现场时间线（证据叙事）**——保留日志毫秒级时间戳（时间|动作|日志表现|结论四列），标注来源（节点/文件）与时钟偏差；有正常对照时优先给对照段（同一时刻正常节点做了什么）

## 变更链路
{改动涉及的代码链路（模块 → 关键文件 → 关键函数），给到其它模块同事能看懂改动落点与影响面的**必要层级**即可，不画交付报告级全量调用链图}

## 核心代码（本次改动/修复）
{列出本次改动/修复的**核心代码片段**（标注 file:line），供其它模块同事快速判断是否影响自己——给关键片段即可，不逐文件全贴、不附完整接口参数表/错误码表。**条件**：跨模块接口变更（签名/行为/协议/错误码变化）时必须贴；纯问题/故障简报（产品主读）可只列关键证据（日志时间点/代码 file:line 指针），不强制贴代码全文，见「反偷懒机制·简报代码条件化」}

## 效果与验证
{最终效果 + 验证概况 + **其它模块需注意的兼容/调用变化**（接口签名变化、行为变化等，如有；无则写"无"）}
```

### TB/日志源简报的强制语义槽位（P0；查BUG/日志/TB 源简报必填，功能开发简报不强制）

**触发**：简报源为 TB 缺陷单或日志分析（metadata `tb_source` 非 null，或交付报告含日志根因分析 `completed_steps` 含 `"log"`）→ 简报**必须**按 [references/external_brief_contract.md §7.1](../references/external_brief_contract.md) 的 6 个语义槽位组织，缺一不可（此强制独立于下述"简报章节都有内容"）：

1. **原现场时间线**（时间|动作|日志表现|结论；保留毫秒；来源=节点/文件+时钟偏差；正常对照优先）——从日志现场时间线**提取**，**不得用当前代码反推现场行为**（现场行为以现场运行版本源码为准，见 log 步骤1「现场运行版本基线门」）
2. **问题点—修复点—验证结果映射**（问题表现 → 对应修复改动 → 验证如何证明修复生效）
3. **修复前后链路**（继承 log_analysis §7.5「修复前后对照」：同图两 subgraph + 新旧关键点表 + 图例；状态边界不混写——"修复前"只描述现场状态、"修复后"只描述部署后的预期；**触发时必填**，低复杂度用 2–4 行对照表并注明"低复杂度不画图"）
4. **验证时间线与版本**（验证发生的时间点 + 验证用代码版本）
5. **版本信息**（现场运行版本 / 当前修复版本 / 验证版本三版本，对应三基线字段 `runtime_code_baselines`/`analysis_code_baselines`/`verification_code_baselines`——分析基线在对外语境表述为"修复所基于版本"，措辞以 [references/external_brief_contract.md §7.1](../references/external_brief_contract.md) 为准；Hash 缺失时显式降级"日志未记录或未能解析现场 Git Hash"，**不得把当前代码版本冒充现场版本**）
6. **统一对外结论口径**（一句话定性 + 归因 + 修复方向，全仓简报口径一致）

**对外简报完成门（机器自检，P0）**：TB/日志源简报生成后按 [references/external_brief_contract.md §10](../references/external_brief_contract.md) 自检——原始时间线存在、复杂故障含修复前后链路、问题—修复—验证映射齐全、`delivery_verdict != verified` 时不得写"整链验证完成"、无 icode 内部产物名 / 无 worktree 清理操作、Hash 以短 Hash 展示；不通过不发简报。

**对外表达契约**：本简报的对外组织与表述（开头结论/现象与影响、归因措辞分级、实现主体/运行载体/下游表现/观测载体四角色拆分、「日志/观测说明」语义拆分、修复与验证状态显式标注、首屏质量门）**统一按 [references/external_brief_contract.md](../references/external_brief_contract.md) 执行**；**TB/日志源简报还须满足该契约 §7.1「必填语义槽位」+ §10「对外简报完成门（机器自检）」，见下方「TB/日志源简报的强制语义槽位」小节**；本段只保留 readme 特有差异（下述交付报告分工、核心代码条件、`delivery_verdict` 状态来源）。
...
- **简报代码条件化**：`_brief.md` 默认含本次改动/修复的**必要代码片段**（标注 file:line）——**跨模块接口变更（签名/行为/协议/错误码变化）时必须贴**，给其它模块同事判断影响面够用即可；**纯问题/故障简报（产品主读）可只列关键证据**（日志时间点/代码 file:line 指针），不强制贴代码全文；两者都不复制交付报告的完整接口参数表/错误码表/测试矩阵/全量调用链图
- **简报禁止编造时间点**：关键时间点必须来自 `created_at` / `patch_history.at` / log 分析记录，无记录写"未记录"
- **简报链路给必要层级**：链路写到"模块 → 关键文件 → 关键函数"即可，不画交付报告级全量调用链图
- **简报语义槽位强制（TB/日志源）**：TB/日志源简报（`_brief.md`）必须含 6 个语义槽位（原现场时间线 / 问题点—修复点—验证结果映射 / 修复前后链路 / 验证时间线与版本 / 版本信息 / 统一对外结论口径，见「TB/日志源简报的强制语义槽位」），缺失任一即不合规，生成后须过「对外简报完成门」；功能开发简报不强制，按通用 6 章节即可
- **简报不写 worktree 收尾信息**：跨领域简报默认**不含** worktree 清理命令/回流操作/绝对路径（worktree 状态段仅交付报告）；简报用"本次变更已合入"等对外口径描述交付状态
...
- 跨领域简报：章节全有实质内容 + 含本次改动/修复核心代码片段（标注 file:line；**跨模块接口变更时必须**，纯问题简报可只列关键证据，见「反偷懒机制·简报代码条件化」）+ 链路给到必要层级 + 时间点真实（无记录写"未记录"）
- **TB/日志源简报（额外）**：含 6 个强制语义槽位（原现场时间线 / 问题—修复—验证映射 / 修复前后链路 / 验证时间线与版本 / 版本信息 / 统一结论口径）+ 过「对外简报完成门」+ 无 worktree 清理命令 + Hash 以短 Hash 展示
- 输出确认行
