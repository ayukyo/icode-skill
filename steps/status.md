...
- `/icode status --verdict <ticket_id> <verified|disproved|superseded> "<reason>" [--correct "<正确方向>"] [--source <machine_test|review|user|auto_signal>] [--superseded-by <ticket_id>]`：手动标注工单方向结论（双写 metadata + 全局索引）
...
**用途**：扫描所有 `verdict=unknown` 的完成态工单（`status=completed`）的 `00_init.md` 末轮对话摘要 + `06_audit.md`，识别含证伪信号的，提示用户标注。解决"旧工单没标 verdict 但可能有坑"的批量治理。**只读 + 提示，不自动写 verdict**（NLP 判方向正确性不可靠，必须用户确认后用 `--verdict` 标注）。

**执行流程**：
1. Read `~/.claude/icode_data/index.json`，`json.load` 全量解析 `tickets` 数组
2. **筛选候选集**：`verdict` 缺失或为 `"unknown"` **且** `status="completed"` **且** `stale=false` 的工单（未完成态跳过--还在流程中；stale 跳过--已过时连避坑都不用）
3. 对每个候选工单：
...
```
=== 扫描结果：N 个 unknown 完成态工单中，M 个疑似含证伪信号 ===

⚠️ 疑似证伪（建议标注 disproved）：
  1. {ticket_id}（{requirement_summary 摘要}）
     信号：末轮「{匹配的信号词 + 上下文片段}」
     建议命令：/icode status --verdict {ticket_id} disproved "{证伪原因}" --correct "{正确方向}"
  2. ...

✅ 无证伪信号（保持 unknown 或后续标 verified）：{K 个工单 id 列表}

ℹ️ 跳过：未完成态 X 个 / stale Y 个
```

5. **不自动写 verdict**：只输出建议命令，用户确认后手动执行 `--verdict` 标注
6. 若无候选（无 unknown 完成态工单），输出"所有完成态工单均已标注 verdict，无需扫描"
