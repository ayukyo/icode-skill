...
索引工单: {全局索引 tickets 数} 条（stale: {stale=true 条数} 条）（用 `json.load` 全量解析 `~/.claude/icode_data/index.json` 的 `tickets` 数组取长度并统计 `stale=true` 数，禁止按行截断--「前 50 行」仅适用于 project_docs 章节）
```
