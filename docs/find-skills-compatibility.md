...
同日继续定位：只对子进程设置Git `http.version=HTTP/1.1`，不改全局配置；
官方update在180秒克隆上限仍退出1。独立执行同源`git clone --depth 1 --branch main`
（300秒外层上限）则自然退出128，报告`curl 56 / GnuTLS recv error (-9)`、
`early EOF`。这证明下载链路也能独立失败，不是ICODE内容检查报错；
不能据此断言HTTP/2、GitHub服务器或某个代理是根因。
再次检查旧6eb09d4安装：328项完整、锁SHA-256与刷新回执一致。
尚未修复官方update；不更换其缓存代码、不关闭TLS校验、不配置不可信镜像。

备用归档下载也须完整验收：175秒预算只收到部分ZIP，续传返回
`curl 33`（服务端不支持Range），均不计成功。部分文件不能交给安装器；
重新下载应写新文件、给予明确总时限，并先做ZIP/固定提交资源完整性检查。
本轮另以410秒重新下载固定提交1fd1749的归档，仍仅收到部分内容、退出28；
因此归档备用路线在当前网络下也未通过，不发布安装命令为“已验证解法”。
本轮停止重复下载，保留现有可信checkout生成完整本地包的既有路线；
网络传输恢复后，再对官方update和远程来源分别重测，不改来源锁伪装成功。
