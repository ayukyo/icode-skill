...
| `skillhub` | 市场/目录 | 导入受阻，尚未发布 | 受阻：页面仓库状态判断与 GitHub 公开状态不一致 | 发布向导已选正确仓库；未形成发布关联 | 未发布，不承诺自动更新 | 2026-09-21 | [官方问题 #2](https://github.com/Tencent/skillhub/issues/2)；另核实候选包 10MiB/200文件与二进制过滤 |
| `skillkit-io` | 市场/目录 | 已提交，页面提示待审核添加 | 完整安装待验 | 提交公开 GitHub 根 URL | 审核与自动刷新未证实 | 2026-09-21 | 等待目录条目，核对来源及资源；勿重复提交 |
| `context7-docs` | 文档索引 | [仓库文档处理中](https://context7.com/ayukyo/icode-skill)；日志已克隆 main | 文档查询待验；技能安装不适用 | 根仓库已关联 | 可处理仓库；定时刷新未证实 | 2026-09-21 | 索引完成后检索具体步骤；不能冒充 Skills 目录收录 |
...
4. **维护现有条目**：SkillsMP 刷新旧描述；Smithery 验证资源与同步；Context7 完成文档索引；SkillKit.io 审核结果。不要重复提交或堆砌关键词。
