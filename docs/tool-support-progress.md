...
| `copilot` | 宿主 | 官方技能目录机制已有依据，见[宿主机制](agent-skill-discovery.md) | 待验，CLI/VS Code/云端不能互相代替认证 | 候选复用单一源码 | 未配置自动更新 | 2026-09-21 | 区别GitHub网页索引、gh技能安装与Copilot真实加载 |
| `cline` | 宿主 | 官方技能目录与Skills CLI接入依据已记录 | 待验，非正式认证 | 候选复用单一源码 | 未配置自动更新 | 2026-09-21 | 按具体版本验证共享技能、MCP、停止点和完整资源 |
| `windsurf-cascade` | 宿主 | 官方文档已转向Devin Desktop/Cascade，见[宿主机制](agent-skill-discovery.md) | 待验，不扩推到全部Devin产品 | 候选复用单一源码 | 未配置自动更新 | 2026-09-21 | 技能加载与MCP Marketplace分开验收 |
| `antigravity` | 宿主 | 官方标准技能目录机制已有依据 | 待验，IDE/CLI/2.0须分别核对 | 候选复用单一源码 | 未配置自动更新 | 2026-09-21 | 不把目录可读当插件上架或运行通过 |
...
2. **验证闭环**：find-skills 的GitHub远程完整安装与显式旧→新刷新已测，update子命令失败单独保留；skill-creator保留所有失败轮、上下文变化和局部补测。末组15项合同符合不代表全部事实正确；模型稳定性、正反触发与真实工具行为仍待验。离线通过、网络受阻和行为缺口分别记录。
