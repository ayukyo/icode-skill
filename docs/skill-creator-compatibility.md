...
此文档只定义评估输入、证据交接与离线检查，不新增 ICODE 命令，不修改 runtime、根 SKILL、许可或安装配置，不调用模型。`tests/skill-evals` 中其他共享技能的成绩不是根 ICODE 的模型评估。
...
无技能组也有事实推断和 Git 解释错误；不能概括为加载技能后全部改善。原始证据和人工复核保存在本地验收记录中，不公开会话及环境细节。本轮交付评估机制与缺口记录，不为单个模型输出修改正确的主流程合同。真实触发、启用工具后的行为和跨模型重复性仍待验证。
