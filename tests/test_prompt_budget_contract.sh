...
if python3 tools/icode_control.py --help 2>&1 | grep -q 'resolve-ticket'; then
  ...
  missing=0
  for cmd in create create-next resolve-ticket validate event step artifact operation policy trace action-policy transition metadata-update index-write index-update migration record-verification record-claim record-skill-run record-agent-spawn record-agent-result archive-manifest close-phase reopen snapshot; do
    printf '%s' "$help" | grep -q "$cmd" || missing=1
  done
  [ "$missing" -eq 0 ] && ok "icode_control.py 25 个控制面子命令可用" || bad "icode_control.py 子命令不完整"
