...
4. **解析所有占位符后再传 prompt** — 主 Agent 在组装子 Agent prompt 时，**必须先将 `{ICODE_OUT_DIR}`、`{当前模型名称}`、`{用户输入的原始需求}` 等占位符替换为实际值**，再将完整的 prompt 传给子 Agent。不可让子 Agent 去猜测目录路径或模型名。
   ...
   **`{用户输入的原始需求}` 的填充规则（关键！）**：
   ...
   - 主 Agent 必须将会话中所有与需求相关的讨论（功能要求、技术约束、用户偏好等）汇总代入
   - 不只是命令参数本身（如"实现计算器"），而是完整需求描述
   - 格式示例：
     ```
     原始命令输入：实现计算器
     对话中补充的需求：
...
5. **禁止主 Agent 接管执行** — 子 Agent 返回结果后，主 Agent 只负责提取内容和更新元信息，**不可自己生成计划/审查报告/代码等内容**。
   ...
   **完成判定标准（三级判定）**：
   ...
   b. 回复中无标记但有实质内容 → 将全部回复内容作为输出，完成（标记不是必需的）
   c. 回复为空或只有报错 → 重新启动子 Agent（使用同一类型），不可自行切换策略
6. **禁止合并或跳过步骤** — 6 个步骤必须逐一执行，**不可合并**（如"快速自检合并步骤5-6"）、**不可跳过**任何步骤。步骤2-6每个都必须启动独立的子 Agent 执行，完成后才能继续下一步。步骤1在主会话中直接执行。

7. **禁止主 Agent 预调研（仅限步骤2-6）** — 启动子 Agent 前，主 Agent **不得自行阅读代码、分析工程结构、预判方案**。子 Agent 的 prompt 中已包含"先了解现有工程"的指令，子 Agent 会自己完成调研。主 Agent 只做：读取步骤文件 → 解析占位符 → 启动子 Agent。**例外：步骤1在主会话中执行，需要主 Agent 阅读代码并直接撰写计划。**
