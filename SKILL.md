...
| `[流程]` `/icode readme` | **可选步骤7**：一次性生成两份——**交付报告**（**给自己看**：完整技术档案，自包含，智能识别功能/查BUG模板）+ **跨领域简报**（`_brief.md`，**给其它模块研发/测试/产品看**：含必要改动/修复代码，主要问题/需求/时间点/链路/修复，较简略；对外表达按统一契约，见 [references/external_brief_contract.md](references/external_brief_contract.md)）。步骤6完成后手动触发 | 用最新目录 |
