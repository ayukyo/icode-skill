...
> **独立调用不要求创建正常工单**：下文两份产物对照是使用场景，不是 `--debug` 的执行前置条件或双跑授权。仅请求 debug 时只处理 debug；明确“不写正式历史”时，不执行也不建议同时新建正常工单，完成提示只保留隔离说明和本次结论，省略“如需正式修复”的旁支建议。
...
- 改为输出 **debug 对照说明**：说明本工单是 debug 孪生（不入索引、不参与主流程、产物仅供与正常工单并列对照研读）。**完成提示条件**：用户明确“不写正式历史”时，省略“如需正式修复”建议；否则可提示“如需正式修复，请用不带 `--debug` 的 `/icode init` / `/icode log` 新建正常工单”，仅解释后续入口，不自动执行（见 [steps/00_init.md](../steps/00_init.md) 步骤9 与 [steps/log.md](../steps/log.md) 步骤11 的 debug 分支）。
