...
- **降级**（没装 / 装了没填三件套）：AI 不替用户判断原生能力
  ...
  - 不报错、不阻塞
- **触发场景**：涉及图片/视频/UI 截图 **（用户主动提供或显式要求时）**—— TB 缺陷源拉取的附件含视频/图片时**默认仅提示**,用户显式说"请分析附件/请看视频/图片"后才调（详见 [steps/log.md](../steps/log.md)「TB 附件提示与可选分析」与「TB 视频/图片附件研读」,反偷懒第 23 条）。**不主动调**的原因:① 防纯文字模型场景默认调报错（模型本身不支持图片/视频,vision-bridge 调用返回空或失败）;② 用户可能不关心附件内容
