...
- **触发场景**：长上下文压缩（log / doc / init / deepcheck / review）、历史工单检索（init / plan / start / fast / log）、模板填充（readme / audit / list）、结构化提取（doc 99_code_facts_audit）、差异摘要（audit 计划vs代码）、远程 README 拉取（doc 模块文档参考输入）—— 22 个入选子任务（单闸门：价值 ≥ 3 ★ + 低风险）
